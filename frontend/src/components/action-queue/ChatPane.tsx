...
export default function ChatPane({ messages, currentPersonId, canSelectOptions, onSelectOption }: ChatPaneProps) {
  ...
  return (
    <Stack spacing={2} sx={{ flexGrow: 1, overflowY: 'auto', py: 1 }}>
      {messages.map((message) => {
        ...
        return (
          <Box key={message.messageId} sx={{ alignSelf: isOwn ? 'flex-end' : 'flex-start', maxWidth: '75%' }}>
                  ...
                  {message.references.map((ref, index) =>
                    ref.type === 'receipt' ? (
                      <Box
                        ...
                        sx={{ width: 56, height: 56, objectFit: 'cover', borderRadius: 1, border: '1px solid #e3e7eb' }}
                      />
                    ) : (
                      <Link key={index} href={ref.url ?? undefined} target="_blank" rel="noreferrer" variant="body2">
                        {ref.label ?? ref.url}
                      </Link>
                    ),
                  )}
                ...
                {message.options.map((option) => {
                  ...
                  return (
                    <Chip
                      ...
                      onClick={canAnswerThis ? () => onSelectOption?.(message.messageId, option.id) : undefined}
                    />
                  );
                })}
              </Stack>
